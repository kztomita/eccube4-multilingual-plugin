...
現状、プラグインインストール後にservices.yamlでmulti_lingual_localesを変更しても正常に動作しません。disableしてから再enableする必要があります。disable時に入力済みのLocale用テキストデータは削除されるので注意してください。変更用のメンテナンスコマンドの追加が必要かも。

その他いろいろ。

当面は、互換性のない変更が入った場合も含め、バージョンは0.0.1のままで更新していきます。
